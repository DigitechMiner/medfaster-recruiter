...
export function Navbar() {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="h-16 flex items-center justify-center px-4 border-b border-gray-100 flex-shrink-0">
          {isExpanded ? (
            <div className="cursor-pointer" onClick={() => router.push("/")}>
              <Image src="/img/brand/new_logo.svg" height={36} width={150} alt="KeRaeva" className="object-contain" priority />
            </div>
          ) : (
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          {navLinks.map(({ href, label, icon: Icon }) => (
            ...
            >
              <Icon className={`w-5 h-5 flex-shrink-0 ${isActive(href) ? "text-[#F4781B]" : "text-gray-400 group-hover:text-gray-600"}`} />
          ...
          <div className="px-3 py-3 border-t border-gray-100 relative" ref={profileRef}>
            ...
            >
              <div className="relative w-8 h-8 flex-shrink-0">
                <Image src="/svg/Photo.svg" alt="Profile" fill className="object-cover rounded-full border-2 border-[#F4781B]" />
                <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-white" />
              </div>
              {isExpanded && (
                <div className="flex-1 min-w-0 text-left">
                  <p className="text-sm font-semibold text-gray-900 truncate">{recruiterProfile?.contact_person_name ?? "—"}</p>
                  <p className="text-xs text-gray-500 truncate">{recruiterProfile?.contact_person_email ?? "—"}</p>
                </div>
              )}
            </button>

            {profileOpen && (
              <div className={`absolute bottom-[calc(100%+8px)] bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden w-56 ${isExpanded ? "left-3 right-3 w-auto" : "left-3"}`}>
                <button onClick={() => { router.push("/profile"); setProfileOpen(false); }} className="flex items-center gap-3 w-full px-4 py-3.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                  <User size={18} className="text-gray-400 flex-shrink-0" />
                  <span className="font-medium">View profile</span>
                </button>
                <button onClick={() => { router.push("/organization"); setProfileOpen(false); }} className="flex items-center gap-3 w-full px-4 py-3.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors border-t border-gray-50">
                  <Building2 size={18} className="text-gray-400 flex-shrink-0" />
                  <span className="font-medium">Organization Details</span>
                </button>
                <div className="border-t border-gray-100" />
                <button onClick={handleLogout} disabled={loggingOut} className="flex items-center gap-3 w-full px-4 py-3.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
      ...
      {/* ── TOP HEADER ── */}
      <header className={`fixed top-0 right-0 h-16 bg-white border-b border-gray-200 flex items-center justify-end gap-3 px-6 z-40 transition-all duration-300 ${isExpanded ? "left-64" : "left-[72px]"}`}>

        <button onClick={() => router.push("/jobs/instant-replacement")} className="flex items-center gap-1.5 border border-[#F4781B] text-[#F4781B] hover:bg-orange-50 rounded-lg px-4 py-1.5 text-sm font-semibold transition-colors whitespace-nowrap h-9">
          <Zap size={14} className="fill-[#F4781B]" />
          <em>Urgent Shift</em>
        </button>

        <button onClick={() => router.push("/jobs/create")} className="flex items-center gap-1.5 bg-[#F4781B] hover:bg-[#e06a10] text-white rounded-lg px-4 py-1.5 text-sm font-semibold transition-colors whitespace-nowrap h-9">
