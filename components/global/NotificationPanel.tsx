...
export const NotificationPanel = ({ onClose }: Props) => {
  ...
  const ensureNotificationsLoaded = useAuthStore(
    (s) => s.ensureNotificationsLoaded,
  );

  const refreshNotifications = useAuthStore(
    (s) => s.refreshNotifications,
  );

  const loadMoreNotifications = useAuthStore(
  ...
  const handleMarkRead = useCallback(
    async (n: Notification) => {
      if (n.is_read || markingRead.has(n.id)) {
        return;
      }
  ...
  const handleNotificationClick = useCallback(
    async (n: Notification) => {
      try {
        ...
        // We still want navigation to work even if
        // marking the notification as read fails.
        console.error(
      ...
      // Get destination based on notification type
    ...
    [handleMarkRead, handleClose, router],
  );
