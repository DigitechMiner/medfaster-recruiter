...
export interface JobApplicationListResponse {
  applications: Array<{
    ...
    job_id:      string;
    candidate_id: string;
    status: 'APPLIED' | 'SHORTLISTED' | 'INTERVIEWING' | 'INTERVIEWED' | 'HIRE' | 'REJECTED' | 'ACCEPTED' | 'CANCELLED'; // ✅ fixed
    created_at:  string;
    updated_at:  string;
    job: {
      id:         string;
...
export async function updateRecruiterJob(id: string, payload: JobUpdatePayload): Promise<JobUpdateResponse> {
...
export async function generateJobDescription(payload: GenerateDescriptionPayload): Promise<GenerateDescriptionResponse> {
...
export async function generateJobQuestions(payload: GenerateQuestionsPayload): Promise<GenerateQuestionsResponse> {
...
export async function getJobFeePreview(params: JobFeePreviewPayload): Promise<JobFeePreviewResponse['data']> {
...
export async function updateApplicationStatus(
  ...
  status: 'APPLIED' | 'SHORTLISTED' | 'INTERVIEWING' | 'INTERVIEWED' | 'HIRE' | 'REJECTED'
) {
  const res = await axiosInstance.patch(ENDPOINTS.JOB_APPLICATION_STATUS(applicationId), { status });
