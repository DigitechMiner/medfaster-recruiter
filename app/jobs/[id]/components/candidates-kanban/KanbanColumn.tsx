...
interface ColProps {
  ...
  onRefetch?:   () => void | Promise<void>;  // ✅ ADDED
...
export function KanbanColumn({
  ...
  const gridCols = isExpanded ? "grid-cols-3" : "grid-cols-1";
      ...
      header={
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {/* ✅ Show real totalCount for applied column, col.count for others */}
            <span className="text-sm text-gray-500">
              {col.key === "applied" && totalCount !== undefined ? totalCount : col.count}
            </span>
          </div>
          <button
            onClick={onToggle}
