...
function toGridCandidate(raw: CandidateListItem) {
  return {
    ...
    role:          raw.specialty?.[0] ?? raw.medical_industry ?? "Healthcare Professional",
...
type GridCandidate = ReturnType<typeof toGridCandidate>;

// ── Main ──────────────────────────────────────────────────────────────────────

export const CandidatesGridView: React.FC<Props> = ({ jobId }) => {
  const { data, isLoading } = useCandidatesList({ page: 1, limit: 12, job_id: jobId });
  ...
  const candidates = (data?.data.candidates ?? []).map(toGridCandidate);
...
export function CandidateGridCard({ candidate: c }: { candidate: GridCandidate }) {
  return (
    <BaseCard
    ...
    >
      <CardHeader className="flex items-center justify-between -pt-1">
        <span className="flex items-center gap-1.5 text-xs font-medium text-green-600 bg-green-50 px-2.5 py-1 rounded-full border border-green-200 w-fit">
      ...
      <CardIdentity className="flex gap-2 pb-1 -mt-2" style={{ alignItems: "flex-start" }}>
        ...
        <div className="flex-1 min-w-0" style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          ...
          <span className="text-xs font-medium text-[#F4781B]" style={{ lineHeight: "1.2", marginTop: "0px" }}>
            {c.role}
          </span>
          ...
          >
            <span style={{ display: "flex", alignItems: "center", gap: "3px" }}>
              <Briefcase size={11} className="text-gray-400" /> {c.exp}
            </span>
            <span className="text-gray-300">|</span>
            <span style={{ display: "flex", alignItems: "center", gap: "3px" }}>
              <Users size={11} className="text-gray-400" /> {c.distance}
            </span>
            <span className="text-gray-300">|</span>
            <span style={{ display: "flex", alignItems: "center", gap: "3px" }}>
              <Star size={11} className="fill-amber-400 text-amber-400" /> {c.rating.toFixed(1)}/5
            </span>
          </CardStats>
        </div>
      </CardIdentity>
