...
export const UrgentJobDetail: React.FC<Props> = ({ job, jobId, onCloseJob }) => {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: job.normalJob?.years_of_experience ?? 'NA',
      sub:   null,
    ...
    },
    {
      ...
      value: formatShiftDate(instant?.start_date),
      sub:   getCountdown(instant?.start_date),
    ...
    {
      label: 'Shift Timings',
      value: instant?.check_in_time && instant?.check_out_time
               ? `${formatTime(instant.check_in_time)} to ${formatTime(instant.check_out_time)}`
               : 'N/A',
      sub:   calcShiftHours(instant?.check_in_time, instant?.check_out_time),
      icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#F4781B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
    },
  ];
  ...
  const infoItems = [
    ...
    {
      icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#F4781B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>,
      text: 'noahliamdoc@gmail.com',
    },
    {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* ── SINGLE CARD: breadcrumb + title + info + specs/quals ── */}
      <div className="bg-white rounded-2xl border border-gray-200 px-6 py-4">

        {/* ── Row 1: ← Jobs / Job_id  ·····  Open | Urgent ── */}
...
<div className="flex flex-wrap items-center gap-2">
  ...
  {hasQuals
    ...
    : <span className="text-sm text-gray-400">N/A</span>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map((card) => (
          <div key={card.label}
            className="bg-white rounded-2xl px-5 py-4 border border-gray-200 flex flex-col gap-1.5 relative">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div>
            <h2 className="text-base text-[#F4781B]">Hired Canadidates</h2>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
              <button onClick={() => setView('grid')} className={`p-1.5 ${view === 'grid' ? 'bg-gray-100 text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}>
                <LayoutGrid size={16} />
              </button>
              <button onClick={() => setView('list')} className={`p-1.5 ${view === 'list' ? 'bg-gray-100 text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}>
                <List size={16} />
              </button>
            </div>
          </div>
        ...
        <div className="bg-[#FEF3E9] px-5 py-3 flex items-center justify-between">
          <button className="w-8 h-8 rounded-lg bg-[#F4781B] text-white text-sm font-semibold flex items-center justify-center">1</button>
  ...
  isOpen={modal.isOpen}
  onClose={modal.close}
  job={job}                          // ← single prop replaces all individual fields
