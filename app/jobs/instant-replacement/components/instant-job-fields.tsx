...
export function InstantJobFields({
  ...
  const selectedTitleLabel = allTitles.find((t) => t.value === formData.jobTitle)?.label ?? "";

  const isCDSW =
    selectedTitleLabel.toLowerCase().includes("community disability support worker") ||
    selectedTitleLabel.toUpperCase().includes("CDSW");

  const neighbourhoodTypes = isCDSW ? CDSW_NEIGHBOURHOOD_TYPES : DEFAULT_NEIGHBOURHOOD_TYPES;

  // ✅ Clear stale neighbourhoodType when job title changes and type is no longer valid
  // Uses a ref-free approach — call updateFormData only when actually invalid
  ...
  const payRateDisplay = payRateCents !== null
    ? `$${(payRateCents / 100).toFixed(2)} / hr`
    : "—";

  return (
    <div className="space-y-6 mb-6">

      {/* Row 1: Requirements & Hourly Pay (read-only) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          <Label htmlFor="hourly-pay" className="text-sm font-medium text-gray-700">
            Hourly Pay per Hire
           
          </Label>
          <div
            id="hourly-pay"
            className="h-11 flex items-center px-3 border border-gray-200 rounded-md
                       bg-gray-50 text-gray-500 text-sm select-none"
          >
            {payRateLoading && (
            ...
            {!payRateLoading && !payRateError && (
              <span className={payRateCents !== null ? "font-semibold text-gray-800" : "text-gray-400"}>
                {formData.jobTitle
                  ? payRateDisplay
