...
export function validateJobPayload(
  ...
  if (duration !== null) {
    if (
      payload.job_urgency === "instant" &&
      duration < INSTANT_JOB_MIN_DURATION_HOURS
    ) {
      push(
        "check_out_time",
  ...
  if (payload.job_urgency === "normal") {
    ...
    const qualResult = isStringArrayBetween(
    ...
    const specResult = isStringArrayBetween(
  ...
  const respResult = isStringArrayBetween(
  ...
  const skillsResult = isStringArrayBetween(
  ...
  if (payload.job_urgency !== "normal") {
    ...
    if (!aiOn) {
      if (hasQuestions && Array.isArray(q) && q.length > 0) {
        push(
          "questions",
          "Questions must be empty when AI interview is disabled.",
        );
      }
    } else {
      if (!Array.isArray(q) || q.length === 0) {
        push(
          "questions",
          "Please add at least one question for the AI interview.",
        );
      } else if (q.length > MAX_QUESTIONS) {
        push("questions", `You can add at most ${MAX_QUESTIONS} questions.`);
      } else if (
        q.some((qq) => typeof qq !== "string" || qq.trim().length === 0)
      ) {
 ...
 * form field keys (camelCase). Used by both InstantJobForm and NormalJobForm
 * to render inline errors next to the right inputs.
 *
 ...
 */
const PAYLOAD_TO_FORM_FIELD: Partial<
  Record<keyof JobCreatePayload, keyof JobFormData>
> = {
  job_title: "jobTitle",
...
export function toFormFieldErrors(
  ...
  for (const err of errors) {
    const formField =
      PAYLOAD_TO_FORM_FIELD[err.field as keyof JobCreatePayload];
