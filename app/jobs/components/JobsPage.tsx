...
const JobsTable: React.FC<{ jobs: any[]; router: any }> = ({ jobs, router }) => {
  return (
...
const JobsPage: React.FC = () => {
  ...
  const [selectedMetric, setSelectedMetric] = useState<MetricType>("openJobs");
  ...
  const tableData = useMemo(() => {
    ...
    const candidates = applicationsData?.applications?.map((app: any) => ({
      ...
      skills: "Patient care, Wound care, IV administration", // You'll get this from API
    ...
    switch (selectedMetric) {
      case "applied":
        return candidates;
      case "interviewing":
        return candidates.filter((c: any) => c.status === "INTERVIEWING");
      case "hired":
        return candidates.filter((c: any) => c.status === "ACCEPTED");
      case "pending":
        return candidates.filter((c: any) => c.status === "PENDING");
      default:
        return [];
    }
  }, [selectedMetric, jobs, applicationsData]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <MetricCard
          ...
          percentChange={15}
          isPositive={true}
          isActive={selectedMetric === "openJobs"}
          onClick={() => setSelectedMetric("openJobs")}
        />
        <MetricCard
          ...
          percentChange={2.5}
          isPositive={true}
          isActive={selectedMetric === "applied"}
          onClick={() => setSelectedMetric("applied")}
        />
        <MetricCard
          title="Candidates in Interview Stage"
          value={metrics.inInterviewStage}
          percentChange={3.2}
          isPositive={true}
          isActive={selectedMetric === "interviewing"}
          onClick={() => setSelectedMetric("interviewing")}
        />
        <MetricCard
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        {selectedMetric === "openJobs" ? (
          <JobsTable jobs={paginatedData} router={router} />
        ) : (
          <CandidatesTable
            candidates={paginatedData}
            showStatus={selectedMetric === "interviewing"}
            showAssignedJob={selectedMetric === "hired"}
            showCurrentEmployer={selectedMetric === "pending"}
            router={router}
          />
        )}
