...
export function CandidateHero({
  ...
  const phone = "************";
  const jobTypes = (
    Array.isArray(candidate.job_type)
      ? candidate.job_type
      : [candidate.job_type]
  )
  ...
  return (
    <div className="rounded-2xl border border-[#e5e7eb] bg-white p-4 md:p-5">
      ...
      <div className="flex flex-col justify-between gap-4 lg:flex-row">
        <div className="flex flex-1 gap-4">
          ...
          <div className="space-y-2">
            <h1 className="text-[34px] leading-none font-semibold text-[#242833]">
              {fullName}
            </h1>
            ...
            <div className="grid grid-cols-1 gap-y-2 text-sm text-[#6B7280] sm:grid-cols-2">
              ...
              <span className="inline-flex items-center gap-1.5">
                ...
                <span className="inline-flex flex-wrap items-center gap-1.5">
                  {(jobTypes.length > 0 ? jobTypes : ["—"]).map(
                    (jobType, index) => (
                      <span
                        key={`${jobType}-${index}`}
                        className="inline-flex rounded-md bg-[#F3F4F6] px-2 py-1 text-xs text-[#525866]"
      ...
      <div className="mt-4 border-t border-gray-200 pt-4 space-y-3">
        ...
        {specs.length > 0 && (
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-medium text-[#F4781B]">
              Specializations :
            </span>
            {specs.map((spec) => (
              <span
