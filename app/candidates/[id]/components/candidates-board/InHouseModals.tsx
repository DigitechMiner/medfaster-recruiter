...
interface AddInHouseModalProps {
  onClose:        () => void;
  onSuccess:      (count: number, entries: StaffEntry[]) => void;
            ...
            <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-4 items-start">
              <div className="flex flex-col gap-1">
                <input
                  ...
                  className={`border rounded-xl px-3.5 py-2.5 text-[13px] text-gray-800 outline-none transition-colors ${
                    errors[i]?.name
              ...
              <button
                ...
                className={`mt-2.5 transition-colors ${
                  entries.length === 1 ? "text-gray-200 cursor-not-allowed" : "text-gray-400 hover:text-red-400"
                }`}
              >
                <X size={16} />
              </button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 px-8 pt-10 pb-8 flex flex-col items-center gap-5 text-center">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[13px] text-gray-400">You will be notified once they are enrolled</p>
        </div>
        <button
          onClick={onDone}
...
export function PartialFailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 px-8 pt-10 pb-8 flex flex-col items-center gap-5 text-center">
        <div className="relative flex items-center justify-center w-24 h-24">
          <div className="absolute inset-0 rounded-full bg-red-100 opacity-50" />
          <div className="absolute inset-3 rounded-full bg-red-100" />
          <div className="relative w-14 h-14 rounded-full bg-white border-2 border-red-400 flex items-center justify-center">
