...
const getActionConfig = (candidate: CandidateWithExtras): ActionConfig => {
  if (candidate.is_ai_recommended)      return { showAiBadge: true,  showShortlist: true,  primaryLabel: "Direct Hire",          primaryVariant: "orange",       actionType: "hire"     };
  if (candidate.is_instant_hire)        return { showAiBadge: false, showShortlist: false, primaryLabel: "Hire Instantly",       primaryVariant: "green",        primaryIcon: "zap",      actionType: "hire"     };
  if (candidate.is_currently_available) return { showAiBadge: false, showShortlist: false, primaryLabel: "Schedule A Interview", primaryVariant: "outline",      primaryIcon: "user",     actionType: "schedule" };
...
export const CandidateHero: React.FC<CandidateHeroProps> = ({
  ...
  const config   = getActionConfig(c);
  ...
  const skillsArray: string[] = useMemo(() => {
    ...
    try {
      return typeof candidate.skill === "string"
        ...
        : Array.isArray(candidate.skill) ? candidate.skill : [];
    } catch { return []; }
  }, [candidate.skill]);

  const roleTags: string[] = useMemo(() => {
    const raw = Array.isArray(c.specialty) ? c.specialty : c.specialty ? [c.specialty] : skillsArray;
    return raw.length > 0 ? raw.slice(0, 3) : ["Licensed Practical Nurse"];
  }, [skillsArray, c]);
  ...
  return (
    <div className="flex flex-col gap-0">
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-0">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <span className="text-[#F4781B] font-semibold">{fullName}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium ${
              ...
              <span className={`w-1.5 h-1.5 rounded-full ${c.is_online ? "bg-green-500" : "bg-gray-400"}`} />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mt-4">
        ...
        <KpiCard label="Preferred Location"    value={prefLocation} />
      </div>
    </div>
  );
};
  ...
  return (
    <div className="flex flex-col sm:items-end gap-2 flex-shrink-0 w-full sm:w-auto">
      <div className="flex flex-wrap justify-start sm:justify-end gap-2">
        <button
          onClick={onExport}
          className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors whitespace-nowrap"
        >
